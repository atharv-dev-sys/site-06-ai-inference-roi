...
const navItems = [
  ...
  { href: "/articles", label: "Articles", icon: <Cpu className="h-5 w-5" /> },
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 shadow-sm">
      <div className="container flex h-16 items-center justify-between">
        <Link href="/" className="flex items-center gap-2" aria-label="InferenceROI Home">
          <Logo />
        </Link>
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent side="right" className="w-full max-w-xs p-6 bg-background">
              <div className="flex items-center justify-between mb-6">
                <Link href="/" className="flex items-center gap-2" aria-label="TechHub Home" onClick={() => setIsMobileMenuOpen(false)}>
                  <Logo />
                </Link>
                <SheetClose asChild>
